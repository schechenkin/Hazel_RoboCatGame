...
void NetworkManagerServer::ProcessPacket( ClientProxyPtr inClientProxy, InputMemoryBitStream& inInputStream )
{
	...
	case kHelloCC:
		...
		HZ_CORE_TRACE("NetworkManagerServer::ProcessPacket::SendWelcomePacket");
...
void NetworkManagerServer::HandlePacketFromNewClient( InputMemoryBitStream& inInputStream, const SocketAddress& inFromAddress )
{
	...
	HZ_CORE_TRACE("NetworkManagerServer::HandlePacketFromNewClient");
	...
	if(  packetType == kHelloCC )
	{
		...
		HZ_CORE_TRACE("NetworkManagerServer::HandlePacketFromNewClient Read name {0}", name);
	...
	else
	{
		...
		HZ_CORE_TRACE("Bad incoming packet from unknown client at socket {0}", inFromAddress.ToString().c_str());
	}
}
...
void NetworkManagerServer::SendWelcomePacket( ClientProxyPtr inClientProxy )
{
	...
	HZ_CORE_TRACE("Server Welcoming, new client '{0}' as player {1}", inClientProxy->GetName().c_str(), inClientProxy->GetPlayerId());
...
{
	//HZ_CORE_TRACE("NetworkManagerServer::RespawnCats");
...
{
	//HZ_CORE_TRACE("NetworkManagerServer::CheckForDisconnects");
...
{
	HZ_CORE_TRACE("NetworkManagerServer::HandleClientDisconnected");
...
{
	HZ_CORE_TRACE("NetworkManagerServer::RegisterGameObject");
...
{
	HZ_CORE_TRACE("NetworkManagerServer::UnregisterGameObject");
